...
from config import DIRICHLET_ALPHA, DIRICHLET_EPS, N_ACTIONS
...
class Node:
    ...
    def add_dirichlet_noise(self, alpha=DIRICHLET_ALPHA, epsilon=DIRICHLET_EPS):
        if not self.children:
            return
        noise = np.random.dirichlet([alpha] * len(self.children)).astype(np.float32)
        for child, n in zip(self.children, noise):
            child.prior = (1.0 - epsilon) * float(child.prior) + epsilon * float(n)
    ...
    def run(self, num_simulation: int):
        for _ in range(num_simulation):
            ...
            if not node.is_terminated:
                ...
                if node is self.root and self.model.training and not hasattr(self.root, "_noise_applied"):
                    self.root.add_dirichlet_noise()
                    self.root._noise_applied = True
            else:
